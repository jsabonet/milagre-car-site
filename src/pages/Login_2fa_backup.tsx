...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Primeira etapa: Iniciar 2FA
      const twoFactorResponse = await authService.initiateTwoFactor(credentials);
      
      if (twoFactorResponse.requires_2fa) {
        // Mostrar tela de 2FA
        ...
        toast({
          ...
          description: "Verifique seu telefone ou email para o código de 6 dígitos.",
          duration: 5000,
        });
        return;
      }
      
      // Se não requer 2FA (fallback para login direto - apenas para desenvolvimento)
  ...
  // Handler para verificação 2FA
  const handleTwoFactorVerification = async (code: string) => {
    setTwoFactorLoading(true);
    setTwoFactorError('');

    try {
      const response = await authService.verifyTwoFactor(sessionToken, code);
      await handleSuccessfulLogin(response);
    } catch (error: any) {
      setTwoFactorError(error.message || 'Código de verificação inválido.');
      setLoginAttempts(prev => prev + 1);
    } finally {
  ...
  // Handler para reenvio de código 2FA
  const handleResendTwoFactor = async () => {
    try {
      await authService.resendTwoFactorCode(sessionToken);
    } catch (error: any) {
      setTwoFactorError(error.message || 'Erro ao reenviar código.');
    }
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
        ...
        <div className="flex items-center justify-center min-h-[calc(100vh-200px)] py-12 px-4 sm:px-6 lg:px-8">
          <TwoFactorAuth
            ...
            phoneNumber="+258 84 *** 5678" // Dados mockados - viria do backend
            email="admin@****car.co.mz"
          />
        </div>
